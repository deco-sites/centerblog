...
export default function Footer({
  ...
  subscribe = {
    title: "Inscreva-se",
    description:
      "Inscreva-se na nossa newsletter para ficar atualizado sobre recursos e lançamentos.",
    instructions:
      "Ao inscrever-se, concorda com a nossa <a href='/' target='_blank' class='link'>Política de Privacidade</a> e dá o seu consentimento para rece",
  },
  madeWith = {
  ...
  return (
    <div class=" pt-16 text-sm bg-[#101728]">
      <div class="lg:container lg:mx-auto md:max-w-6xl mx-4 pt-16 text-sm bg-[#101728]">
            ...
            <div class="flex gap-9">
              {links?.map((link) => (
                <div>
                  ...
                  {link.items?.map((item) => (
                    <a
                      class="block link no-underline py-1 text-white"
                      href={item.href}
                    >
                      {item.label}
                    </a>
          ...
          <div class="border-primary border-t flex flex-col gap-4 items-center justify-between lg:flex-row lg:items-center py-8">
            <div class="flex flex-col gap-4 items-center lg:flex-row lg:gap-6">
              <a
                ...
                target="_blank"
              >
                <span class="text-sm">{madeWith?.label}</span>
                <Image
                  src={madeWith?.src || ""}
                  width={100}
                  height={28}
              ...
              <span class="text-white text-sm">{copyright}</span>
              <div class="flex gap-2 justify-between lg:gap-6">
                {bottomLinks?.map((item) => (
                  <a class="link text-white text-sm" href={item.href} target="_blank">
                    {item.label}
                  </a>
                ))}
              </div>
